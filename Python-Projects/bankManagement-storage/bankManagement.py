...
def logged_in_menu(accnt,name):
	...
	rootwn.title("Hai Nguyen Bank | Welcome - "+name)
	...
	b2=tk.Button(image=myimg2,command=lambda: display_Cr(accnt,name))
	b2.image=myimg2
	b3=tk.Button(image=myimg3,command=lambda: display_De(accnt,name))
	b3.image=myimg3
	b4=tk.Button(image=myimg4,command=lambda: disp_bal(accnt))
	b4.image=myimg4
	b5=tk.Button(image=myimg5,command=lambda: disp_trans(accnt))
	b5.image=myimg5
...
def Main_Menu():
	rootwn=tk.Tk()
	rootwn.geometry("1600x500")
	rootwn.title("Bank Management System - 	HaiNguyen")
	rootwn.configure(background='SteelBlue1')
	fr1=tk.Frame(rootwn)
	fr1.pack(side="top")
