...
Version: 01
Language: python 
Tasks: personal aliexpress items scraper 
"""
# -*- coding: utf-8 -*-
from tkinter import *
import tkinter as tk, webbrowser, sys, pyperclip
#Must install pyperclip
...
# I Keep this Space for THE METHODS
def searchAli() :
	website = input_text.get()
	if len(website) > 1:
		webbrowser.open('https://m.aliexpress.com/popular/'+ website +'.html')
	else:
		inputnotfound();
def hello ():  
...
def inputnotfound ():
	label3 = tk.Label(root, text= 'Please give me an Input', fg='green', font=('helvetica', 12, 'bold'))
	canvas1.create_window(150, 200, window=label3)
def userinput ():
	label2 = tk.Label(root, text= 'alibaba!', fg='green', font=('helvetica', 12, 'bold'))
	canvas1.create_window(150, 200, window=label2)
def openlinkgrab ():
	#Get address from clipboard.
	addressurl = pyperclip.paste()
	if not addressurl.startswith('http'):
...
button1 = tk.Button(text='Click Me',command=userinput, bg='brown',fg='white')
canvas1.create_window(150, 150, window=button1)
button2 = tk.Button(text='Search Ali',command=searchAli, bg='brown',fg='white')
canvas1.create_window(150, 100, window=button2)
button3 = tk.Button(text='Open Grabbed Link',command=openlinkgrab, bg='brown',fg='white')
canvas1.create_window(150, 50, window=button3)
